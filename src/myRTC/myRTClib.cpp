...
DS1302::DS1302(uint8_t SCLK, uint8_t IO, uint8_t CE){
  DS1302_SCLK_PIN = SCLK;
  DS1302_IO_PIN = IO;
  DS1302_CE_PIN = CE;
}

//fucntion to set the time and date of DS1302
void DS1302::setDS1302Time(int year, int month, int dayofmonth, int dayofweek, int hours, int minutes, int seconds)
{      
  ds1302_struct rtc;
...
//get the time, return as a time_t
time_t DS1302::getDS1302Time()
{
  ds1302_struct rtc;
  // Read all clock data at once (burst mode).
  DS1302_clock_burst_read( (uint8_t *) &rtc);
  tmElements_t tmSet;
  tmSet.Year = 2000 + bcd2bin( rtc.Year10, rtc.Year) - 1970;
  tmSet.Month = bcd2bin( rtc.Month10, rtc.Month);
  tmSet.Day = bcd2bin( rtc.Date10, rtc.Date);
  tmSet.Hour = bcd2bin( rtc.h24.Hour10, rtc.h24.Hour);
  tmSet.Minute = bcd2bin( rtc.Minutes10, rtc.Minutes);
  tmSet.Second = bcd2bin( rtc.Seconds10, rtc.Seconds);
